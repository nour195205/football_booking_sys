...
<div class="container-fluid py-3 px-3 mb-5">
    <form action="{{ route('admin.fields.update', $field->id) }}" method="POST">
        ...
        <div class="mb-4">
            <label class="section-title"><i class="fas fa-futbol me-2 text-primary"></i>اسم الملعب</label>
            <input type="text" name="name" class="form-control form-control-lg shadow-sm" value="{{ $field->name }}" required placeholder="مثلاً: ملعب التوفيق">
        </div>
        ...
        <div id="prices-container">
            @foreach($field->prices as $index => $price)
            <div class="price-card shadow-sm border-0">
                ...
                <div class="row g-3 mt-1">
                    <div class="col-6">
                        ...
                        <input type="time" name="prices[{{$index}}][from]" class="form-control form-control-lg" value="{{ $price->from_time }}" required>
                    </div>
                    <div class="col-6">
                        <label class="small text-muted fw-bold">إلى (وقت)</label>
                        <input type="time" name="prices[{{$index}}][to]" class="form-control form-control-lg" value="{{ $price->to_time }}" required>
                    </div>
                    <div class="col-12">
                        <label class="small text-muted fw-bold">سعر الساعة (ج.م)</label>
                        <input type="number" name="prices[{{$index}}][price]" class="form-control form-control-lg" value="{{ $price->price }}" required placeholder="0.00">
                    </div>
                    <div class="col-12">
                        <label class="small text-muted fw-bold">اسم الفترة (اختياري)</label>
                        <input type="text" name="prices[{{$index}}][label]" class="form-control form-control-lg" value="{{ $price->label }}" placeholder="مثلاً: الفترة المسائية">
                    </div>
                </div>
            </div>
            @endforeach
        ...
        <div class="sticky-bottom-actions d-flex gap-2">
            <button type="submit" class="btn btn-primary btn-lg w-100 rounded-3 fw-bold shadow">تحديث البيانات</button>
            <a href="{{ route('admin.fields.index') }}" class="btn btn-outline-secondary btn-lg w-50 rounded-3 fw-bold">إلغاء</a>
        </div>
    </form>
</div>

<script>
    let rowIdx = {{ $field->prices->count() }};
    document.getElementById('add-price-row').addEventListener('click', function() {
        ...
        newCard.innerHTML = `
            ...
            <div class="row g-3 mt-1">
                ...
                <div class="col-12">
                    <label class="small text-muted fw-bold">اسم الفترة (اختياري)</label>
                    <input type="text" name="prices[${rowIdx}][label]" class="form-control form-control-lg" placeholder="مثلاً: الفترة الصباحية">
                </div>
            </div>
        `;
        container.prepend(newCard); // إضافة الفترة الجديدة في الأعلى لسهولة رؤيتها
        rowIdx++;
    });
